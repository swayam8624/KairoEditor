...
export namespace kairo::editor
{
    ...
    public:
        ...
        [[nodiscard]] kairo::renderer::CameraPose ViewportCamera() const noexcept
        {
            const auto pose = m_ViewportController.Pose();
            ...
            {
                const auto& authored = m_Project.Scene().Camera(*m_ViewportSceneCamera);
                result.Projection = authored.Projection == kairo::engine::CameraProjection::Orthographic
                    ? kairo::renderer::CameraProjectionMode::Orthographic
                    : kairo::renderer::CameraProjectionMode::Perspective;
                result.VerticalFovRadians = authored.VerticalFovRadians;
                result.OrthographicSize = authored.OrthographicSize;
                result.NearPlane = authored.NearPlane;
                result.FarPlane = authored.FarPlane;
            }
            return result;
        }
        ...
        /// Output: one host-level request to launch the project's real runtime.
        /// Play is intentionally distinct from the in-editor physics simulation.
        [[nodiscard]] bool TakeRuntimeLaunchRequest() noexcept
        {
    ...
    private:
        ...
        bool m_RuntimeLaunchRequested = false;
        ...
        void DrawViewport()
        {
                ...
                ImGuiWindowFlags_NoScrollWithMouse))
            {
                ...
                ImGui::GetWindowDrawList()->AddText({ overlay.x, overlay.y + 18.0f }, IM_COL32(135, 165, 184, 190),
                    "MMB/Option+LMB orbit | Shift+MMB pan | wheel dolly | RMB+WASD fly | Shift+WASD / arrows move | Home free | C camera");
                const auto selected = m_State.SelectedEntity();
                if (selected.has_value())
                {
        ...
        void FocusSelection()
        {
            ...
            m_ViewportController.Focus(m_Project.Scene().WorldTransform(*selected).Translation);
        }

        void ViewSceneCamera()
        {
            const auto& scene = m_Project.Scene();
            auto cameraEntity = scene.PrimaryCamera();
            if (!cameraEntity.has_value())
            {
                const auto cameras = scene.CameraEntities();
                if (!cameras.empty()) cameraEntity = cameras.front();
            }
            if (!cameraEntity.has_value())
                return;

            const auto world = scene.WorldTransform(*cameraEntity);
        ...
        void HandleViewportNavigation(bool hovered, bool keyboardActive)
        {
            ...
            if (input.Fly)
            {
                if (rightMouse) ImGui::SetMouseCursor(ImGuiMouseCursor_None);
                input.MoveForward =
                    (ImGui::IsKeyDown(ImGuiKey_W) || ImGui::IsKeyDown(ImGuiKey_UpArrow) ? 1.0f : 0.0f) -
        ...
        [[nodiscard]] bool DrawTransformGizmo(ImVec2 viewportMin, ImVec2 viewportSize)
        {
            ...
            const auto camera = ViewportCamera();
            const Mat4f view = LookAt(camera.Position, camera.Target, camera.Up);
            kairo::renderer::ShowcaseCamera projectionCamera;
            projectionCamera.SetPose(camera);
            Mat4f projection = projectionCamera.Projection(
                static_cast<std::uint32_t>(std::max(viewportSize.x, 1.0f)),
                static_cast<std::uint32_t>(std::max(viewportSize.y, 1.0f)));
